...
def close():
    ...
    global idling, extracting, flashing
...
def save_extraction(
    ...
    tempSetPoint = [tkSetPoint.get()] * len(temperatureLst)
...
def check_profile(
    ...
    customProfile['settings']['name'] = tkProfileName.get()
    ...
    if customProfile['settings']['name'] in profileLst:
...
def assign_profile(
    ...
    config['settings']['profile'] = <profile>['name'].
    """
    ...
    config['settings']['profile'] = profile['settings']['name']
...
def start(
    ...
    global idling, extracting, flashing, counter, temperatureLst, pressureLst
    global tkCounter, tkTempValue, tkPresValue
...
def count(
    ...
    global extracting, counter, temperatureLst, pressureLst
    global tkCounter, tkTempValue, tkPresValue
...
def calculate_profile_quartile(
    ...
    profileRange = list(
        np.arange(
            profileMin,
            float(customProfile['settings']['extractionDuration']),
            0.1
        )
    )
...
def scale_callback(
    ...
    Description
    ---------------------------------------------------------------------
    Updates the scale label and the available temperature set-point
    values within the settings form when the scale value is changed.
    """
...
def setpoint_callback(
    ...
    if config['settings']['scale'] == 'F':
        ...
        elif config['tPID']['setPoint'] > 105:
            config['tPID']['setPoint'] = 105
        else:
            pass
    try:
        utils.write_config(
...
def create_profile(
    ...
    if (
        ...
        tkProfileName.set(customProfile['settings']['name'])
        ...
        infusionDurationLst = list(range(0, 4))
